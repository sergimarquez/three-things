 ...
 *
 * WHY THIS EXISTS:
 * Previously, every component calling useEntries() would independently:
 * 1. Read from localStorage
 * 2. Parse JSON
 * 3. Validate data
 * 4. Create its own React state
 *
 * With this context:
 * - Data is loaded ONCE at the app root (in EntriesProvider)
 * - All components share the same state via React Context
 * - No duplicate localStorage reads or parsing
 *
 * HOW TO USE:
 * 1. Wrap your app with <EntriesProvider> (done in main.tsx)
...
type EntriesContextType = {
  ...
  getYearEntries: (year: string) => Entry[];
  getEntriesForMonth: (month: string) => Entry[];
  getYearStarredItems: (year: string) => ReturnType<typeof getStarredItems>;
  getStarredItemsForMonth: (month: string) => ReturnType<typeof getStarredItems>;
  getMonthlyFavoritesForYear: (year: string) => ReturnType<typeof getMonthlyFavoritesUtil>;
  ...
  shouldShowYearlyReviewPrompt: () => boolean;
  getPreviousMonth: () => string;
  getMonthsNeedingReview: () => string[];
...
export function useEntriesContext(): EntriesContextType {
